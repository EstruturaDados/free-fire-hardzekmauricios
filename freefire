...
typedef struct {
    ...
    char tipo[TAM_TIPO];   // Tipo do componente (ex: "controle", "suporte")
    int prioridade;        // Prioridade (1 a 10)
} Componente;

// -------------------------------------------------------
// Variável global para contar comparações de cada algoritmo
// -------------------------------------------------------
...
// Remove '\n' que o fgets guarda no final da string, se existir
...
// Lê um inteiro de forma simples e consome o '\n' restante
int lerInteiro() {
    int valor;
    scanf("%d", &valor);
    getchar(); // consome o '\n' que ficou no buffer
...
// Cada um retorna o tempo de execução em segundos (double)
// e usa a variável global contadorComparacoes
// -------------------------------------------------------
...
double insertionSortTipo(Componente v[], int n) {
    ...
    for (i = 1; i < n; i++) {
        ...
        // Enquanto j >= 0 e v[j].tipo > chave.tipo (ordem alfabética)
        while (j >= 0) {
            contadorComparacoes++;
...
// -------------------------------------------------------
// Retorna o índice do elemento encontrado ou -1 se não achar
...
// -------------------------------------------------------
// (Opcional) Função genérica de medição de tempo
// Exemplo de assinatura pedida no enunciado
// -------------------------------------------------------
//
...
// }
//
// Aqui não usamos diretamente porque cada sort já
// retorna o tempo e manipula contadorComparacoes.
// -------------------------------------------------------
...
int main() {
    ...
    int qtd = 0;
    int opcao;
    int ordenadoPorNome = 0;  // flag: 1 se a lista estiver ordenada por nome
    ...
    do {
        ...
        switch (opcao) {
            ...
            case 2: {
                ...
                ordenadoPorNome = 1;  // agora está em ordem por nome
